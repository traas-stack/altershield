...
CREATE TABLE `altershield_meta_defender_rule` (
   ...
   `change_filter_ref` varchar(65535) DEFAULT NULL comment 'Defense rule filter conditions',
   PRIMARY KEY(`id`),
   KEY `idx_plugin_key`(`plugin_key`) GLOBAL,
   KEY `idx_defense_range_key`(`defense_range_key`) GLOBAL,
   KEY `idx_status`(`status`) GLOBAL,
   KEY `idx_range_key_status`(`defense_range_key`, `status`) GLOBAL
) DEFAULT CHARSET = utf8mb4 COMMENT = 'Defender rules table';

CREATE TABLE `opscloud_exe_defender_detect` (
    ...
    `result_ref` varchar(65535) DEFAULT NULL comment 'Details of this detection result',
